...
if [ "$LEVEL" = "deep" ]; then
  ...
  printf '\n=== gate: architecture ===\n'
  ARCH_FAIL=0
  if rg -n 'NEXT_PUBLIC_TMDB' app components lib; then
    c_red "architecture: TMDB credentials must never use a NEXT_PUBLIC_ variable"
    ARCH_FAIL=1
  fi
  if git ls-files '.env' '.env.local' '.env.*.local' | grep -q .; then
    c_red "architecture: a local environment file is tracked by Git"
    ARCH_FAIL=1
  fi
  if ! rg -q '^import "server-only";' lib/tmdb.ts; then
    c_red "architecture: lib/tmdb.ts must retain its server-only boundary"
    ARCH_FAIL=1
  fi
